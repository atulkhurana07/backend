...
async def run_migrations_and_seed():
    ...
    async with session_factory() as db:
        ...
        # 2. Seed Real Bikaner EV Charging Centers
        bikaner_stations = [
            {
        ...
        for b in buses_seed:
            ...
            dev_obj = d_res.scalar_one_or_none()
            if not dev_obj:
                dev_obj = Device(
            ...
            if not t_obj:
                ...
                t_obj.soc_pct = b["soc"]
                t_obj.observed_at = datetime.now(timezone.utc)
        ...
        help_contacts = [
            ...
            {"city": "Bikaner", "state": "Rajasthan", "department": "Bikaner Traffic Police Control Room", "category": "emergency", "phone": "+91 151 222 6100", "availability": "24/7 Emergency", "description": "Traffic updates, road clearance, and emergency transit assistance."}
        ]

        for hc in help_contacts:
            await db.execute(text("""
                INSERT INTO help_contacts (id, city, state, department, category, phone, availability, description, public_visible)
                SELECT 
                    gen_random_uuid(),
                    ...
                    CAST(:availability AS VARCHAR),
                    CAST(:description AS VARCHAR),
                    TRUE
                WHERE NOT EXISTS (
                    SELECT 1 FROM help_contacts WHERE department = CAST(:department AS VARCHAR)
                );
            """), hc)
        await db.commit()
    ...
    print("[SUCCESS] Full migration and seed completed!")
